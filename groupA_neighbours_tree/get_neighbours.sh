cp ../groupA_transmission/groupA_PlusTransmission.fasta tree.fasta

tail -n+2 ../mask_2/groupA/L.closest.csv | cut -d"," -f2 | cut -d";" -f 1-100 | tr ";" "\n" | sort | uniq > L.IDs.txt
tail -n+2 ../mask_2/groupA/R.closest.csv | cut -d"," -f2 | cut -d";" -f 1-100 | tr ";" "\n" | sort | uniq > R.IDs.txt
